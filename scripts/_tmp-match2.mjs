...
const folder = 'C:/Users/PedroCastanho/Downloads/Eventos (3)';
...
matched.forEach(m => console.log(`  "${m.file}" -> [${m.item.temporada} / ${m.item.arco}${m.item.subarco ? ' / ' + m.item.subarco : ''}] ${m.item.name} (já tinha imagem: ${!!m.item.imageUrl})`));

console.log(`\n=== AMBÍGUOS (${ambiguous.length}) ===`);
ambiguous.forEach(a => {
  console.log(`  "${a.file}" bate com ${a.candidates.length} itens:`);
  a.candidates.forEach(c => console.log(`     - [${c.temporada} / ${c.arco}${c.subarco ? ' / ' + c.subarco : ''}] ${c.name} (id=${c.id}, imagem atual: ${c.imageUrl ? 'sim' : 'não'})`));
});
